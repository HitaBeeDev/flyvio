...
export function getLayoverMinutes(left: FlightSegment, right: FlightSegment) {
  return Math.round(
    (new Date(right.departureTime).getTime() - new Date(left.arrivalTime).getTime()) / 60_000,
  )
}
...
export function getFleetType(flight: Flight) {
  return flight.outbound[0]?.aircraft ?? flight.inbound?.[0]?.aircraft ?? 'Fleet information unavailable'
}
...
export function getFareBreakdown(
  ...
  const childrenDiscount = adults === 0 && children > 0
    ? Math.round(pricePerTraveler * children * 0.14)
    : Math.round(adultBaseFare * children * 0.25)
  const taxes = Math.max(
    45,
    pricePerTraveler * Math.max(adults + children, 1) - adultFareTotal - childFareTotal + childrenDiscount,
  )
