...
import type { DepartureWindow, FilterState, Flight, FlightSegment, SearchParams, SortOption, StopFilter } from '@/types'
...
export function formatResultsMeta(params: SearchParams) {
  ...
  const travelers = [
    ...
    params.passengers.infants ? `${params.passengers.infants} Infant${params.passengers.infants > 1 ? 's' : ''}` : null,
  ]
    .filter(Boolean)
    .join(', ')
...
export function getAirlineCounts(flights: Flight[]) {
  return flights.reduce<Record<string, { code: string; name: string; count: number }>>(
    (accumulator, flight) => {
...
function matchesDepartureWindow(departureTime: string, windows: DepartureWindow[]) {
  if (windows.length === 0) {
    return true
  }
  ...
  return windows.some((windowKey) => {
    const window = DEPARTURE_WINDOWS.find((candidate) => candidate.key === windowKey)
...
export function filterFlights(flights: Flight[], filters: FilterState) {
  return flights.filter((flight) => {
    ...
    return (
      ...
      matchesDepartureWindow(flight.outbound[0]!.departureTime, filters.departureWindows) &&
      (filters.airlines.length === 0 || filters.airlines.includes(flight.airline.code)) &&
...
export function sortFlights(flights: Flight[], sort: SortOption) {
  return [...flights].sort((left, right) => {
    ...
    if (sort === 'fastest') {
      return getJourneyDuration(left.outbound) - getJourneyDuration(right.outbound)
    }
    ...
    const leftScore = left.stops * 180 + getJourneyDuration(left.outbound) + left.price / 8
    const rightScore = right.stops * 180 + getJourneyDuration(right.outbound) + right.price / 8
...
export function parseFilters(searchParams: URLSearchParams, defaults: FilterState): FilterState {
  ...
  return {
    ...
    airlines: airlines ? airlines.split(',').filter(Boolean) : defaults.airlines,
