...
function getSeedFlight(
  ...
  const flight = generateFlightsForRoute(origin, destination, departureDate, cabinClass, isRoundTrip)[index]

  if (!flight) {
    throw new Error(`Missing seed flight at index ${index} for ${origin}-${destination}`)
  }
