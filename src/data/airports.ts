...
function isSouthAmericaTimezone(timezone: string) {
  return southAmericaTimezonePrefixes.some((prefix) => timezone.startsWith(prefix))
}
...
export const airportsByCode = new Map(airports.map((airport) => [airport.iata, airport]))
